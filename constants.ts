...
export const MOCK_USERS: User[] = [
  ...
  { id: 'user-3', name: 'Rohan Mehta', role: UserRole.STUDENT, institutionId: 'inst-1', class: 'Grade 8, Section A', avatarUrl: 'https://picsum.photos/seed/rohan/100/100', rollNumber: '8A-02' },
  { id: 'user-4', name: 'Sanya Gupta', role: UserRole.STUDENT, institutionId: 'inst-1', class: 'Grade 8, Section A', avatarUrl: 'https://picsum.photos/seed/sanya/100/100', rollNumber: '8A-03' },
  { id: 'user-5', name: 'Kabir Verma', role: UserRole.STUDENT, institutionId: 'inst-1', class: 'Grade 8, Section B', avatarUrl: 'https://picsum.photos/seed/kabir/100/100', rollNumber: '8B-01' },
  { id: 'user-6', name: 'Diya Patel', role: UserRole.STUDENT, institutionId: 'inst-1', class: 'Grade 8, Section B', avatarUrl: 'https://picsum.photos/seed/diya/100/100', rollNumber: '8B-02' },
  { id: 'user-7', name: 'Ishaan Reddy', role: UserRole.STUDENT, institutionId: 'inst-1', class: 'Grade 8, Section B', avatarUrl: 'https://picsum.photos/seed/ishaan/100/100', rollNumber: '8B-03' },
];
...
const generateLabScore = (moduleId: string, score: number): LabScore => ({
    moduleId,
    score,
    steps: [], // Simplified for mock data
    completedOn: new Date().toISOString()
});

export const MOCK_STUDENT_PROGRESS: Record<string, StudentProgress> = {
  'user-1': { // Aarav Sharma (High-achiever)
    quizScores: {
      'quiz-1': { quizId: 'quiz-1', score: 5, totalQuestions: 5 },
      'quiz-2': { quizId: 'quiz-2', score: 5, totalQuestions: 5 },
      'quiz-3': { quizId: 'quiz-3', score: 4, totalQuestions: 5 },
      'quiz-4': { quizId: 'quiz-4', score: 7, totalQuestions: 7 },
    },
    labScores: {
      'mod-1': generateLabScore('mod-1', 90),
      'mod-2': generateLabScore('mod-2', 95),
      'mod-3': generateLabScore('mod-3', 85),
    },
    timeSpent: 8.5,
  },
  'user-3': { // Rohan Mehta (Average)
    quizScores: {
      'quiz-1': { quizId: 'quiz-1', score: 4, totalQuestions: 5 },
      'quiz-2': { quizId: 'quiz-2', score: 3, totalQuestions: 5 },
    },
    labScores: {
      'mod-1': generateLabScore('mod-1', 80),
    },
    timeSpent: 4.2,
  },
  'user-4': { // Sanya Gupta (Struggling)
    quizScores: {
      'quiz-1': { quizId: 'quiz-1', score: 2, totalQuestions: 5 },
    },
    labScores: {},
    timeSpent: 2.1,
  },
  'user-5': { // Kabir Verma (Good start)
    quizScores: {
      'quiz-1': { quizId: 'quiz-1', score: 5, totalQuestions: 5 },
      'quiz-2': { quizId: 'quiz-2', score: 4, totalQuestions: 5 },
      'quiz-3': { quizId: 'quiz-3', score: 5, totalQuestions: 5 },
    },
    labScores: {
      'mod-1': generateLabScore('mod-1', 75),
      'mod-2': generateLabScore('mod-2', 80),
    },
    timeSpent: 6.8,
  },
  'user-6': { // Diya Patel (All modules complete)
    quizScores: Object.fromEntries(QUIZZES.map(q => [q.id, { quizId: q.id, score: Math.floor(Math.random() * (q.questions.length - 3 + 1)) + 3, totalQuestions: q.questions.length }])),
    labScores: Object.fromEntries(MODULES.map(m => [m.id, generateLabScore(m.id, Math.floor(Math.random() * (100 - 75 + 1)) + 75)])),
    timeSpent: 15.0,
  },
  'user-7': { // Ishaan Reddy (No progress)
    quizScores: {},
    labScores: {},
    timeSpent: 0.5,
  },
