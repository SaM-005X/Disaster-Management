    ...
    useEffect(() => {
        ...
        if (studentData.length > 0) {
            ...
            studentData.forEach(student => {
                const progress = progressData[student.id] || { quizScores: {}, labScores: {}, points: 0, badges: [] };
        ...
        <div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md overflow-hidden">
                <div className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="border-b border-gray-200 dark:border-gray-700">
                        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                            <button onClick={() => setActiveTab('roster')} className={`${activeTab === 'roster' ? 'border-teal-500 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-lg`}>{rosterTitle}</button>
                            <button onClick={() => setActiveTab('leaderboard')} className={`${activeTab === 'leaderboard' ? 'border-teal-500 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-2 px-1 border-b-2 font-medium text-lg`}>{translate('Leaderboard')}</button>
                        </nav>
                    </div>
                     <button
                ...
                <div className="p-6">
                            ...
                            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3">{translate('Points')}</th>
                                ...
                                <tbody>
                                    {studentProgressList.length > 0 ? studentProgressList.map(student => (
                                        <tr key={student.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            ...
                                            <td className="px-6 py-4 font-semibold text-amber-600 dark:text-amber-400">{student.points.toLocaleString()}</td>
                    ...
                    {activeTab === 'leaderboard' && <Leaderboard students={studentData} progressData={progressData} currentUser={user} />}
                </div>
            </div>
            {isModalOpen && (
                <StudentEditModal 
