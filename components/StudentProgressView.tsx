...
const StudentProgressView: React.FC<StudentProgressViewProps> = ({ user, modules, progress }) => {
    ...
    const chartData = useMemo(() => {
        if (completedModulesCount === 0 && progress.timeSpent < 0.1) {
            return [{ x: 0, y: 0 }, { x: 1, y: 0 }]; // Show an empty chart baseline if no time is spent
        }
        const points = [{ x: 0, y: 0 }];
        const timePerModule = completedModulesCount > 0 ? progress.timeSpent / completedModulesCount : 0;
        for (let i = 1; i <= completedModulesCount; i++) {
            points.push({ x: timePerModule * i, y: i });
        }
        // Ensure the line extends to the full time spent if it's more than the last completed module's time.
        if (progress.timeSpent > (points[points.length - 1]?.x || 0)) {
    ...
    useEffect(() => {
        ...
        textsToRead.push({ id: 'progress-student-subheader', text: translate('Track your journey to becoming Surksha.') });
        ...
        modules.forEach(module => {
            ...
            const quizText = translate('Learning Module & Quiz');
